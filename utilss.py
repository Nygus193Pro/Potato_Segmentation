...
# nazwy klas ziemniaków
class_names = ['ziemniak', 'bez lupiny', 'pekniecie', 'ksztalt',
               'pomarszczony', 'kielkujacy', 'zaschniete', 'przeciety']

# kolory losowane dla każdej klasy
rng = np.random.default_rng(3)
colors = rng.uniform(0, 255, size=(len(class_names), 3))


def nms(boxes, scores, iou_threshold):
    # nms – usuwanie nakładających się detekcji
...
def xywh2xyxy(x):
    # konwersja bbox (x,y,w,h) → (x1,y1,x2,y2)
    y = np.copy(x)
    y[..., 0] = x[..., 0] - x[..., 2] / 2
    y[..., 1] = x[..., 1] - x[..., 3] / 2
    y[..., 2] = x[..., 0] + x[..., 2] / 2
...
def draw_masks(image, boxes, class_ids, mask_alpha=0.3, mask_maps=None):
    ...
    for i, (box, class_id) in enumerate(zip(boxes, class_ids)):
        ...
        if mask_maps is None:
            ...
            crop_mask_img = crop_mask_img * (1 - crop_mask) + crop_mask * blue_color
            mask_img[y1:y2, x1:x2] = crop_mask_img
    return cv2.addWeighted(mask_img, mask_alpha, image, 1 - mask_alpha, 0)


def draw_comparison(img1, img2, name1, name2, fontsize=2.6, text_thickness=3):
    # łączenie dwóch obrazów z podpisami
